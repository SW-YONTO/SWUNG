...
export async function getCopilotToken(accessToken) {
  ...
  if (!response.ok) {
    ...
    let errorMsg = `Failed to get Copilot token: ${response.status}`;
    
    try {
      const errorJson = JSON.parse(errorText);
      if (errorJson.error_details && errorJson.error_details.notification_id === 'spammy_user') {
        throw new Error('GitHub has temporarily blocked this account for excessive login attempts ("spammy_user"). Please wait 30-60 minutes before trying again, or revoke the "GitHub for VS Code" authorization in your GitHub settings.');
      }
      errorMsg += ` - ${errorJson.message || errorText}`;
    } catch (e) {
      if (e.message.includes('spammy_user')) throw e; // Re-throw our custom error
      errorMsg += ` - ${errorText}`;
    }
